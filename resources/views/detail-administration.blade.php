					...
					<ul class="uk-list uk-list-divider">
						<?php  $presentations = App\Pages::where('tag','presentation')->get(); ?>
								@if($presentations->count() > 0)
										<li><a class="uk-button-link" style="text-decoration : none !important ;" href="{{url('/vous-etes',[$tag,'slug'=>App\Pages::where('slug','precautions-elementaires')->first()->slug])}}"> {{App\Pages::where('slug','precautions-elementaires')->first()->titre}}</a></li>
										<li><a class="uk-button-link" style="text-decoration : none !important ;" href="{{url('/vous-etes',[$tag,'slug'=>App\Pages::where('slug','principales-menaces')->first()->slug])}}"> {{App\Pages::where('slug','principales-menaces')->first()->titre}}</a></li>
										<li><a class="uk-button-link" style="text-decoration : none !important ;" href="{{url('/vous-etes',[$tag,'slug'=>App\Pages::where('slug','bonne-pratiques')->first()->slug])}}">{{App\Pages::where('slug','bonne-pratiques')->first()->titre}}</a></li>
								@endif
					</ul>
				</div>
				</div>
